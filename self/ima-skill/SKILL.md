...
# Load skill version from meta.json (located next to this SKILL.md)
SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
IMA_SKILL_VERSION="$(python3 -c "import json,sys;print(json.load(open(sys.argv[1]))['version'])" "$SKILL_DIR/meta.json" 2>/dev/null || echo "unknown")"
```
...
ima_api() {
  ...
  curl -s -X POST "https://ima.qq.com/$path" \
    ...
    -H "ima-openapi-ctx: skill_version=$IMA_SKILL_VERSION" \
